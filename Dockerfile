FROM node:9.3-slim
LABEL author="quesadaaba@gmail.com"
WORKDIR /tmp
COPY .docker/webdriver-version.js ./
...
RUN npm install -g protractor minimist && \
    node ./webdriver-version.js --chromedriver 2.32 && \
    ...
    rm ${CHROME_PACKAGE} && \
    mkdir /protractor
COPY .docker/protractor.sh /
COPY .docker/enviornment /etc/sudoers.d/
